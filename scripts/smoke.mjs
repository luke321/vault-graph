...
const all = [];
/*
 * github#113 -- every check says where it runs and on which clock, and the default is the
 * cheap one: the demo vault only, with the page's animations at FAST_CLOCK. A check claims
 * more when its assertion is ABOUT the fixture's shape (`on: "all"`, or a list of fixture
...
// github#7, github#113 -- the frame-sensitive lane is gone: with one Chrome at a time there
// is no contention to keep those checks away from, so every check on one fixture shares one
// Chrome, and a check that reads frames says so with `clock: "real"` instead of by name.
...
check("a folder named after an Object.prototype member still lays out", async (p, ctx) => {
  ...
  try {
    ...
    // github#113 -- the fixture page arrives with its opening camera tween; this check
    // navigated, so the arrival is its to wait out
...
check("layout matches its golden snapshot", async (p) => {
  ...
  // github#113 -- a switch keeps the rings it was switched into (decisions/0011), so the
  // relayout above, taken in the tag dimension, would otherwise leave every later check on
  // this page inside tag-derived rings; the folder disc re-derives its own before this returns
  if (dim !== "folder") {
...
check("tags: each dimension keeps its own hidden and collapsed state", async (p) => {
  const r = await p.j(`(function(){
    ...
    __vg.setDim("tag");
    var tagAgain = live();
    // leave the page as the shard found it -- in BOTH dimensions. hiddenSub is swapped per
    // dimension on a switch (that is what this check asserts), so the tag disc's copy has to
    // be cleared while the tag disc is up: cleared only after the switch, "(untagged)/" stayed
    // hidden on the tag disc and every later switch to it drew nothing (github#113 -- found
    // when the live-rebuild checks stopped getting a Chrome of their own).
...
check("a marked heatmap day haloes but never pushes", async (p) => {
  ...
  // github#113 -- the mark's halo ramps in and releases on the page's own clock
  await settle(p);
...
check("a folder's legend row toggles \"hidden by default\" from its context menu", async (p) => {
  const r = await p.j(`(function(){
    ...
    // would be the identical mistake it exists to catch. Restored the way it was flipped:
    // a second click on the same menu item. Writing folderShown and the hidden flag back
  ...
  // github#113 -- the two clicks' cascades: the second cancels the first, and walks
  await settle(p);
...
async function runOne(vault, work) {
  ...
  try {
    ...
    // github#113 -- the page arrives with its opening camera tween (and, without ?rest, the
    // intro) still running; that is the page's own doing, not the first check's, so it is
    // waited out here once. From here on, what is busy at a check boundary is the check's.
    ...
    for (const c of mine) {
      ...
      // has FAILED, whatever it asserted: the next check would read a layout mid-cascade
      // (github#112 was exactly that -- a raw plan read inheriting a stale splitHold). The
      // cascade is waited out here all the same, so one leak never fails two checks.
...
async function main() {
  ...
  // github#113 -- one Chrome per fixture on the resting page, plus one for the intro check
  // where it runs; a check runs on a fixture only when it says so (runsOn)
  const jobs = [];
  ...
  const ran = new Map();
  // github#113 -- every check's cost, per fixture, for --timings <file>
  const timingsOut = [];
  const bump = (work, r) => {
