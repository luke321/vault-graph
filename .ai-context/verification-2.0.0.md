...
| **Size dots from the frame**, merged for 2.0.0 and on by default | github#41, design/0011 | all three | cascades; and one tight resting pair after a hide | off → on, measured: rest, search and solo identical to the pixel; demo hidden state one dot 0.14 px smaller at 1.08 (0.44 px at 0.35), 0 px over the bar; goldens byte-identical; mid-walk a dot may be held below its two resting sizes (trough 0.8 % / 20.9 % / 1.1 %), never above |
