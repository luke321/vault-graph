...
| **Unlinked notes join their folder** | On by default: a note with no links takes its own folder's wedge and colour, the same as any other note in it. Turn this off to keep unlinked notes together instead, as one separate population with a flat grey swatch. The `(unlinked)` row in the graph's own legend carries the same toggle either way — it's the very last row in the list (after `(vault root)`, matching where a loose note sits in Obsidian's own file explorer), greyed out while it holds nothing. |
