...
| **Unlinked notes join their folder** | On by default: a note with no links takes its own folder's wedge and colour, the same as any other note in it. Turn this off to keep unlinked notes together instead, as one separate population with a flat grey swatch. The `(unlinked)` row in the graph's own legend carries the same toggle either way — while this is on it stays in the legend at 0, greyed out and pushed to the very end since there's nothing in it right now; turn it off from there (or here) and it jumps back up to sit right after any archive folders, at full colour. |
